...
例如，通过如下实例（[参考 Kubernetes 官方文档](https://kubernetes.io/docs/concepts/workloads/controllers/replicaset/)）[<sup>[1]</sup>]，我们可以利用Kubernetes 副本集（ReplicaSet）机制，自动部署推理系统的多个实例。用户在以下 yaml 文件中，配置副本集（ReplicaSet）选项。
```yml
apiVersion: apps/v1
kind: ReplicaSet
metadata:
...
例如，我们可以通过下面的实例练习（[参考 TorchServe 文档](https://pytorch.org/serve/grpc_api.html)[<sup>[2]</sup>]），将训练好的模型部署并通过 [gRPC](https://grpc.io/) 的 API 调用。这样意味着，我们可以将模型本身变为微服务，融合进当前已有的基础架构中其他的微服务中只需要客户端提供好相应的输入数据，处理好返回数据即可，模型本身当成一个软件服务被使用。
...
- 模型中间表达标准（[ONNX](https://onnx.ai/)[<sup>[3]</sup>]）：让框架，工具和运行时有一套通用的模型标准，使得优化和工具能够被复用。
...
- 模型转换工具（[MMdnn](https://github.com/microsoft/MMdnn)[<sup>[4]</sup>]）：让模型可以打通不同框架已有工具链，实现更加方便的部署或迁移学习（Transfer Learning）。
...
<center>图 8.3.4 通过 MMdnn 模型转换(Model Convert) [图片引用 MMdnn 文档](https://github.com/microsoft/MMdnn)</center>
...
<center>图 8.3.5 边缘部署和云端部署关系 [图片引用 Jiasi, et al 论文](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=8763885)</center>
...
如图 8.4.5 Jiasi 等对边缘侧部署做过[相关综述](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=8763885)[<sup>[5]</sup>]，通过 1~5 子图可以观察到边缘部署和推理方式常见的为以下几种：
...
<center>图 8.3.6 边缘部署和推理方式总览 [图片引用 Jiasi, et al 论文](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=8763885)</center>
